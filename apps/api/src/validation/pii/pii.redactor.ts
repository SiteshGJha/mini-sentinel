...
export class PiiRedactor {
  ...
  async redactParametersAsync(params: Record<string, any>): Promise<{
    ...
    try {
      ...
      this.logger.warn(`PII microservice TCP query failed: ${err.message}. Falling back to local heuristics.`);
    }
  ...
  private inferPiiTypeFromKey(key: string): PIIMatch['type'] {
    ...
    if (k.includes('card') || k.includes('cc') || k.includes('pan')) return 'CREDIT_CARD';
