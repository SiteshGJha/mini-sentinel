...
export class PiiDetector {
  ...
  private patterns = {
    ...
    CREDIT_CARD:
      /\b(?:3[47]\d{2}(?:[ -]?\d{6}){2}|(?:4\d{3}|5[1-5]\d{2}|6011)(?:[ -]?\d{4}){3})\b/g,
    BANK_ACCOUNT: /\b\d{8,17}\b/g, // Bank account numbers are 8 to 17 digits
    ADDRESS:
      /\b\d{1,5}\s+[a-zA-Z0-9\s.,#]{5,40}\s+(?:Street|St|Avenue|Ave|Road|Rd|Boulevard|Blvd|Drive|Dr|Lane|Ln|Court|Ct|Circle|Cir|PO\s+Box)\b/gi,
  ...
  detect(content: string): PIIMatch[] {
    ...
    while ((match = this.patterns.BANK_ACCOUNT.exec(content)) !== null) {
      ...
      if (!isOverlapping) {
        ...
        const hasContext =
          /account|bank|checking|savings|routing|acc|dep/i.test(contextWindow);
        ...
        matches.push({
          ...
          confidence: hasContext ? 0.85 : 0.4,
        });
      }
    }
