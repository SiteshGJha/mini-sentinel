...
export class RuleEngine {
  ...
  async evaluate(request: ParsedRequest): Promise<{
    ...
    if (
      params.contactAttemptsLast7Days !== undefined &&
      params.calls_last_7_days === undefined
    ) {
      params.calls_last_7_days = Number(params.contactAttemptsLast7Days);
    }
    if (
      params.localTime !== undefined &&
      params.contact_hour_local === undefined
    ) {
    ...
    if (
      params.ceaseContact !== undefined &&
      params.cease_communication_requested === undefined
    ) {
      params.cease_communication_requested = !!params.ceaseContact;
    }
    if (
      params.bankruptcyHold !== undefined &&
      params.bankruptcy_hold === undefined
    ) {
    ...
    for (const rule of rules) {
      ...
      try {
        if (
          rule.type === RuleType.REG_F ||
          rule.name.toLowerCase().includes("bankruptcy")
        ) {
          if (request.tool.name !== "dialer") {
            status = "PASS";
            checkResult = {
              message: "Skipped: Collections rule only applies to dialer tool.",
            };
          } else {
          ...
          if (
            (ruleName.includes("dti") ||
              ruleName.includes("credit score") ||
              ruleName.includes("employment")) &&
            request.tool.name !== "loan-underwriter"
          ) {
            status = "PASS";
            checkResult = {
              message: `Skipped: Underwriting rule '${rule.name}' only applies to loan-underwriter.`,
            };
          } else {
            const sampleResult = this.evaluateSampleRuleConditions(
              request,
              rule,
            );
          ...
          if (request.tool.name !== "loan-underwriter") {
            status = "PASS";
            checkResult = {
              message: "Skipped: QM rule only applies to loan-underwriter.",
            };
          } else {
  ...
  private mapRuleTypeToValidationType(type: RuleType) {
    switch (type) {
      case RuleType.REG_F:
        return "REG_F_COMPLIANCE";
      case RuleType.QM:
        return "QM_COMPLIANCE";
      case RuleType.OFAC:
        return "OFAC_COMPLIANCE";
      case RuleType.PII:
  ...
  private evaluateSampleRuleConditions(
    ...
    const failDetails = (rule.actionParams as any) || {
    ...
    for (const cond of conditions) {
      ...
      if (type === "threshold") {
        ...
        if (
          val === undefined &&
          (field === "debt_to_income_ratio" || field === "dti")
        ) {
          const monthlyIncome = this.getNestedValue(
            request,
            "parameters.monthlyIncome",
          );
          const monthlyDebt = this.getNestedValue(
            request,
            "parameters.monthlyDebt",
          );
        ...
        if (!ok) {
          return {
            ...
            reason:
              failDetails.message ||
              `Field '${field}' failed threshold ${op} ${targetVal}.`,
        ...
        if (!ok) {
          return {
            ...
            reason:
              failDetails.message ||
              `Field '${field}' is outside range [${min}, ${max}].`,
        ...
        if (expr === "not cease_communication_requested") {
          const cease = this.getNestedValue(
            request,
            "parameters.cease_communication_requested",
          );
          ok = !cease;
        } else if (expr === "not consumer_represented_by_attorney") {
          const represented = this.getNestedValue(
            request,
            "parameters.consumer_represented_by_attorney",
          );
          ok = !represented;
        } else if (expr === "not third_party_recipient") {
          const thirdParty = this.getNestedValue(
            request,
            "parameters.third_party_recipient",
          );
          ok = !thirdParty;
        }
        ...
        if (!ok) {
          return {
            passed: false,
            reason:
              failDetails.message || `Expression constraint failed: ${expr}`,
    ...
    return {
      passed: true,
      reason: "",
      details: { message: "All conditions met." },
    };
  }

  // Reg F Rules Dispatcher
  private async evaluateRegF(
    request: ParsedRequest,
    rule: ComplianceRule,
    ...
    return {
      passed: true,
      reason: "",
      details: { message: "Skipped rule check" },
    };
  }
  ...
  private evaluateRegFTimeWindow(request: ParsedRequest, rule: ComplianceRule) {
    ...
    const passed =
      timeInMinutes >= minInMinutes && timeInMinutes < maxInMinutes;

    return {
      passed,
      reason: passed
        ? ""
        : `Contact outside permitted hours (08:00 - 21:00). Actual: ${localTime}`,
      details: { localTime, hour, minute },
    };
  }

  private async evaluateRegFCallFrequency(
    request: ParsedRequest,
    rule: ComplianceRule,
  ) {
    const params = request.parameters || {};
    ...
    if (!customerId) {
      return {
        passed: true,
        reason: "",
        details: { message: "No customerId, frequency check skipped" },
      };
    }
    ...
    return {
      ...
      reason: passed
        ? ""
        : `Reg F contact attempts limit of ${limit} exceeded. Actual: ${count} contact attempts.`,
      details: { contactCount: count, limit, customerId },
    };
  }

  private evaluateRegFCeaseContact(
    request: ParsedRequest,
    rule: ComplianceRule,
  ) {
    const params = request.parameters || {};
    const ceaseContact =
      params.ceaseContact !== undefined
        ? params.ceaseContact
        : params.cease_communication_requested;
    const passed = !ceaseContact;
    return {
      passed,
      reason: passed
        ? ""
        : "Cease contact request is in force for this customer.",
  ...
  private evaluateBankruptcyHold(request: ParsedRequest, rule: ComplianceRule) {
    const params = request.parameters || {};
    const bankruptcyHold =
      params.bankruptcyHold !== undefined
        ? params.bankruptcyHold
        : params.bankruptcy_hold;
  ...
  private evaluateRegFCooldown(request: ParsedRequest, rule: ComplianceRule) {
    ...
    const passed =
      daysSinceLastConv === undefined || Number(daysSinceLastConv) >= 7;
    return {
      passed,
      reason: passed
        ? ""
        : `7-day post-conversation cooldown in force. Days since last contact: ${daysSinceLastConv}`,
  ...
  private evaluateQM(request: ParsedRequest, rule: ComplianceRule) {
    ...
    if (dti === undefined) {
      return {
        ...
        reason: "",
        details: {
          message: "No DTI or Debt/Income params provided, skipped check",
        },
      };
    }
    ...
    return {
      ...
      reason: passed
        ? ""
        : `Qualified Mortgage DTI threshold of ${threshold}% exceeded (Actual: ${parsedDti.toFixed(2)}%)`,
  ...
  private evaluateOFAC(request: ParsedRequest, rule: ComplianceRule) {
    const fullName = this.getNestedValue(
      request,
      rule.field || "parameters.fullName",
    );
    if (!fullName || typeof fullName !== "string") {
      return {
        passed: true,
        reason: "",
        details: { message: "No fullName provided, skipped OFAC check" },
      };
    }

    const sanctionedNames: string[] = this.configService.get(
      "ofac_sanctions",
      [],
    );
    ...
    return {
      ...
      reason: !isSanctioned
        ? ""
        : `OFAC Sanction Match detected for name: ${fullName}`,
  ...
  private evaluateGeneric(request: ParsedRequest, rule: ComplianceRule) {
    ...
    if (value === undefined) {
      return {
        passed: true,
        reason: "",
        details: { message: `Field ${rule.field} not found, skipped` },
      };
    }
    ...
    switch (rule.operator) {
      ...
      case RuleOperator.CONTAINS:
        passed = String(value)
          .toLowerCase()
          .includes(String(ruleVal).toLowerCase());
        break;
      ...
      case RuleOperator.IN:
        passed =
          Array.isArray(ruleVal) && ruleVal.map(String).includes(String(value));
        break;
      case RuleOperator.NOT_IN:
        passed =
          Array.isArray(ruleVal) &&
          !ruleVal.map(String).includes(String(value));
        break;
      default:
        passed = true;
    }
    ...
    return {
      ...
      reason: passed
        ? ""
        : `Rule constraint violated: ${rule.field} must satisfy ${rule.operator} ${JSON.stringify(ruleVal)}`,
      details: {
        field: rule.field,
